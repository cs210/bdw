...
#pragma mark - IDApplicationDataSource protocol definition
...
/*!
 @abstract Asks the datasource to provide the manifest for the IDApplication.
 @discussion The source of the manifest is a plist file. The manifest contains metadata about the application (e.g. name, version, vendor), the AppModel and signatures. The AppModel is required by the AppSwitcher. This model contains the information that is shown in the AppSwitcher, e.g. a 50*50 px png-icon, the name translated to the 23 supported HMI languages for i18n purposes, the application URL, ... The signatures are created by BMW and verify that your application has been reviewed by BMW and is allowed to connect to vehicles. The application manifest dictionary returned from the delegate will be cached within IDApplication. Setting the datasource delegate again will flush the cached manifest.
 @param application An object representing the application requesting this information.
 @return dictionary representation of the AppSwitcher model
 */
...
/*!
 ...
 @abstract Asks the datasource to return the HMI description for a Remote HMI application.
 @discussion A HMI service will only be created if the datasource provides an hmi description.
 @param application An object representing the application requesting this information.
 @return  A NSData object containing the HMI description.
 */
- (NSData *)hmiDescriptionForApplication:(IDApplication *)application;

/*!
 @method textDatabasesForApplication:
 @abstract Asks the datasource to return the text databases for a Remote HMI application.
 @discussion The resulting array can contain multiple text databases to be used. The application determines which text databases it needs to provide depending on the properties brand and hmiType of the currently connected vehicle (refer to @link //apple_ref/occ/cl/IDVehicleInfo @/link). If multiple databases contain a text resource with the same identifier, the text resource in the database with the highest index in the returned array will be used. The Remote HMI supports uploading of up to five image databases. If more than five image databases are returned, the hmi will reject the connection attempt.
 @param application  An object representing the application requesting this information.
 @return  An array of NSData objects containing the text databases.
 */
- (NSArray *)textDatabasesForApplication:(IDApplication *)application;

/*!
 @method imageDatabasesForApplication:
 @abstract Asks the datasource to return the image databases for a Remote HMI application.
 @discussion The resulting array can contain multiple image databases to be used. The application determines which image databases it needs to provide depending on the properties brand and hmiType of the currently connected vehicle (refer to @link //apple_ref/occ/cl/IDVehicleInfo @/link). If multiple databases contain an image with the same identifier, the image in the database with the highest index in the returned array will be used. The Remote HMI supports uploading of up to five image databases. If more than five image databases are returned, the hmi will reject the connection attempt.
 @param application An object representing the application requesting this information.
 @return  An array of NSData objects containing the image databases.
 */
- (NSArray *)imageDatabasesForApplication:(IDApplication *)application;

@end

#pragma mark - IDApplicationDelegate protocol definition
...
/*!
 ...
 @discussion Connected means that the a communication channel between the application and the vehicle is established. The CDS service (refer to @link //apple_ref/occ/cl/IDCdsService @/link) and the HMI service (refer to @link //apple_ref/occ/cl/IDHmiService @/link) (if required by the application) are registered with the remote HMI and ready to use. This is the right place to register for CDS values and HMI events. Be aware that the HMI service is only registered, the HMI itself has not been started yet. The AUDIO service (refer to @link //apple_ref/occ/cl/IDAudioService @/link) has not yet been registered.
 @param application the application instance that did connect to the vehicle (refer to @link IDApplication @/link)
 @param vehicleInfo some information about the connected vehicle (refer to @link //apple_ref/occ/cl/IDVehicleInfo @/link)
 */
...
/*!
 ...
 @param application the application instance that did start (refer to @link  IDApplication @/link)
 */
...
/*!
 ...
 @discussion The application has been stopped. All services have been shut down (CDS, HMI, AUDIO). There is no need to de-register from CDS values or unsubscribe from HMI events.
 @param application the application instance that did stop (refer to @link IDApplication @/link)
 */
...
/*!
 ...
 @abstract Initialization method for a new IDApplication instance.
...
/*!
 ...
 @abstract Start the remote application asynchronously. The application delegate gets notified whether the startup was successful or did fail.
...
/*!
 @property connected
 @abstract Information about the connection status of the app. The flag will be set to yes if the application was started successfully.
 */
@property (assign, readonly, getter = isConnected) BOOL connected;

/*!
 @property hmiService
 @abstract The application's IDHmiService instance
 @discussion This property is available when the IDApplication is started
 */
@property (strong, readonly) IDHmiService *hmiService;

/*!
 @property cdsService
 @abstract The application's IDCdsService instance
 @discussion This property is available when the IDApplication is started and use of CDS was specified within the IDApplicationManifest
 */
@property (strong, readonly) IDCdsService *cdsService;

/*!
 @property audioService
 @abstract The application's IDAudioService instance
 @discussion This property is available when the IDApplication is started and use of audio was specified within the IDApplicationManifest
 */
@property (strong, readonly) IDAudioService *audioService;
 ...
 */
@property (nonatomic, weak) id<IDApplicationDelegate> delegate;

/*!
 @property dataSource
 @abstract The datasource must implement the @link IDApplicationDataSource @/link protocol. The datasource will not
 be retained by the IDApplication. This property is not KVO compliant.
 */
@property (nonatomic, weak) id<IDApplicationDataSource> dataSource;
